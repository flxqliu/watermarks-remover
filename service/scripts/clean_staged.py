...
def _changed(result: dict) -> bool:
    ...
    Text cleaning reports explicit removal/replacement statistics. For binary and
    container cleaners, actions that removed nothing append a 'nothing was
    removed' filler message (issue #173), so a non-empty actions list alone cannot
    indicate change. A change is confirmed when byte counts differ, explicit
    change flags are set, or active modifying actions (e.g. dropping chunks,
    blanking XMP packets) are present in the report.
    """
