"""Multi-scheme support in the MarkLLM text harness (gap 05-A2 / A1).

CI-covered assertions that the scheme surface extended for the v1 study
remains backward compatible: new scheme keys are additive, the bench's
default is still synthid, and the rewrite loop accepts the new schemes.
"""
...
def test_scheme_map_covers_v1_study_schemes() -> None:
    assert SCHEMES["kgw"] == "KGW"
    assert SCHEMES["synthid"] == "SynthID"
    assert SCHEMES["synthid-text"] == "SynthID"
    # New in the v1 study (research/01 §2): EXP (Gumbel), Unigram, SIR.
